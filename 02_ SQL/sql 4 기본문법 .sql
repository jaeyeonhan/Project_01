...
#LIKE;원하는 값을 일부 키워드만드로 조회 
SELECT * FROM customer WHERE first_name LIKE 'A%'; #시작하는
SELECT * FROM customer WHERE first_name LIKE '%A'; #끝나는
SELECT * FROM customer WHERE first_name LIKE '%A%'; # a include
SELECT * FROM customer WHERE first_name LIKE '%RA'; #
SELECT * FROM customer WHERE first_name NOT LIKE '%A%'; # a not inclue 
...
SELECT * FROM CTE WHERE col_1 LIKE '%#%%' ESCAPE '#'; # #. & ! / 도 사용할 수 있다. -> %%% = %가 들어간 데이터를 검색

SELECT * FROM customer WHERE first_name LIKE 'A_'; #문자열 길이 2
SELECT * FROM customer WHERE first_name LIKE 'A__'; #문자열 길이 3 A로 시작
SELECT * FROM customer WHERE first_name LIKE '__A';#문자열 길이 3 A로 끝남
SELECT * FROM customer WHERE first_name LIKE '_____'; #길이 5글자
SELECT * FROM customer WHERE first_name LIKE 'A_R%'; #A_R로 시작하고 몇글자인지 상관 없음
SELECT * FROM customer WHERE first_name LIKE '__R%';
SELECT * FROM customer WHERE first_name LIKE 'A%R_';
SELECT * FROM customer WHERE first_name REGEXP '^K|N$'; # K로 시작하거나  N으로 끝나는 
...
SELECT * FROM customer WHERE first_name LIKE '_______' AND first_name REGEXP 'A[L-N]' AND first_name REGEXP 'O$'; #총 7 글자이고 A 뒤에 L , N사이에 글자가 있고. 마지막 글자는 o
...
SELECT special_features, rating FROM film GROUP BY special_features, rating HAVING rating = 'G'; #그룸화 후 rating 열에서 G인 데이터만 필터링
SELECT special_features, COUNT(*) AS cnt FROM film GROUP BY special_features HAVING cnt > 70;
SELECT distinct special_features, rating FROM film; #그룹화 하거나 계산 작업 즉 count 할 수 없다.반드시 group by 절을 사용해야 한다.          
